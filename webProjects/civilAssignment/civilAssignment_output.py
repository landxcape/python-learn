...
while(time_axis < total_time):
    ...
    if(time_axis % vehicle_interval == 0):
        ...
        v_id = f'vehicle-{entry_time}'
        v = Vehicle(v_id=v_id)
        v.position(1)
        v.travel_time(travel_time - 1)
        vehicle_in.append(v)
    ...
    # removing vehicles from node 1 list that reached node 2 (queue)
    vehicle_in = list(set(vehicle_in) - set(queue))
